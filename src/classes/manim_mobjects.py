...
from manim import *
import numpy as np
from classes.manim_utils import *
import json
import pandas as pd
...
class Particle(Ellipse):
    """
    A class for creating the small particles/cells that can be seen in the background of the video in the very beginning.
    """
...
class Membrane(VMobject):
    """
    This class is used to create the membrane of the cell. It contains the logic to make the cell wobble using the sine function.
    """
...
class Nucleus(Graph):
    """
    Used to create the core of the cell, i.e. the neural network inside the cell. 
    It contains the logic to create the neural network graph, adjust its edge opacities according to their activation as well as
    the logic to add edges to the graph and to create all necessary mobjects to represent the genotype of the neural network.
    """
...
class Cell(VGroup):
    """
    Used to create the cells throughout the animation. Cells consist of a Membrane (i.e. the outline) and a Nucleus (i.e. the NN within). 
    """
...
class Closeup():
    """
    This class is similar to the Cell class, but it is used to create a closeup of the nucleus of a cell, 
    which is shown during the second half of the video in the top right corner.
    Here the cells nodes are labeled and the membrane is rectangular in order to make space for the labels.
    It contains a method to launch the lunar lander gym environment and to visualize the neural network's activation.
    """
    def __init__(
            self, 
            run,
