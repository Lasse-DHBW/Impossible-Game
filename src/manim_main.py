from manim_mobjects import Cell, Nucleus, ColorChangeAnimation, Particle, Closeup
from manim_utils import read_file, decypher_genotype, calc_lag_ratio, CText
from manim import *
import pandas as pd
import random
...
# Color schema
colors = {
    "highlight": "#EBA607",  # Yellow # Obacht: Bei Änderung auch in manim_mobjects Nucleus LabeledDot anpassen
...
class SceneFullSimulation(Scene00Setup):
    def construct(self):
        ...
        Right out of the gate, one can see that the individual possesses two hidden layers, each containing one node. Let us now take a look at the connections, starting with the strongest one. 
        The edge connecting nodes 5 and 10, links the angle to the horizontal thrust. This is the same connection that species 0 had in generation 34, aiding the aircraft in maintaining balance.
        The next strongest link connects the y-velocity to the second hidden node, which then ties to the vertical thrust. 
        This setup allows the aircraft to decelerate its descent in proportion to its current velocity.
        ...
        There are also some connections whose purpose is not immediately clear. For example the connections linking the bias and right leg to the first hidden node, which in turn influences the horizontal thrust.
